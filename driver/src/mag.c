...
void f3d_mag_init() {
  ...
  f3d_i2c1_write(0x3C, 0x00,  &value,I2C1); // Mag (0x3C), CRA (0x00) 

  value = 0xE0;                      // +/- 8.1 Gauss Full Scale
  f3d_i2c1_write(0x3C, 0x01, &value,I2C1); // Mag (0x3C), CRB (0x01)

  value = 0x00;                      // Continuous Conversion
  f3d_i2c1_write(0x3C, 0x02, &value,I2C1); // Mag (0x3C), MR  (0x23)
}
...
void f3d_mag_read(float *mag_data) {
  ...
  f3d_i2c1_read(0x3C, 0x03, buffer,1);   // Read X Axis
  f3d_i2c1_read(0x3C, 0x07, buffer+2,1); // Read Y Axis
  f3d_i2c1_read(0x3C, 0x05, buffer+4,1); // Read Z Axis (notice that Z is out of order in the chip). 
